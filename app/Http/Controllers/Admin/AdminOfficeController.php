...
class AdministrationController extends Controller
{
    ...
    public function groupDelete(Request $request)
    {
        ...
        try {
            ...
            AdminGroup::findOrFail($request->id)->delete();
    ...
    public function groupSort(Request $request)
    {
        try {
            ...
            foreach ($request->order as $pos => $id) {
                AdminGroup::where('id', $id)->update(['position' => $pos + 1]);
            }
    ...
    public function officeDelete(Request $request)
    {
        ...
        try {
            ...
            AdminOffice::findOrFail($request->id)->delete();

            DB::commit();
            Session::flash('success', 'Office deleted successfully.');
        } catch (\Exception $e) {
            DB::rollBack();
            Session::flash('error', 'Failed to delete office: ' . $e->getMessage());
        }
    ...
    public function officeSort(Request $request)
    {
        try {
            ...
            foreach ($request->order as $pos => $id) {
                AdminOffice::where('id', $id)->update(['position' => $pos + 1]);
            }
