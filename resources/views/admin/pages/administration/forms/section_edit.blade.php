<x-admin-app-layout :title="'Edit Section - ' . $office->title">
    <div class="card shadow-sm">
        <div class="card-header d-flex align-items-center justify-content-between">
            <h3 class="card-title fw-bold mb-0">Edit Section</h3>
        ...
        <div class="card-body">
            <form action="{{ route('admin.administration.section.update') }}" method="POST" novalidate>
                @csrf
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="section_type" class="form-select form-select-sm">
                            ...
                            <option value="overview" @if (old('section_type', $section->section_type) == 'overview') selected @endif>Overview</option>
                            <option value="message" @if (old('section_type', $section->section_type) == 'message') selected @endif>Message</option>
                            <option value="officer_cards" @if (old('section_type', $section->section_type) == 'officer_cards') selected @endif>Officer Cards</option>
                            <option value="alumni_cards" @if (old('section_type', $section->section_type) == 'alumni_cards') selected @endif>Alumni Cards</option>
                            <option value="download_list" @if (old('section_type', $section->section_type) == 'download_list') selected @endif>Download List</option>
                    ...
                    @php
                        $extra = is_array($section->extra) ? $section->extra : (json_decode($section->extra ?? '', true) ?? []);
                    @endphp

                    <div class="col-md-3 mb-3">
                        <label class="form-label fw-semibold">Tab Label</label>
                        <input type="text" name="tab_label" class="form-control form-control-sm"
                            value="{{ old('tab_label', $extra['tab_label'] ?? '') }}" placeholder="Left menu label...">
                    </div>
                    ...
                    <div class="col-md-12 mb-3">
                        ...
                        <small class="text-muted">If you need custom advanced data, paste JSON here. It will be merged with Tab fields above.</small>
                    </div>
                </div>
