    ...
    @push('scripts')
        ...
        <script>
            (function() {
                ...
                function uploadFileToEditorEndpoint(file) {
                    var formData = new FormData();
                    formData.append('file', file);

                    return fetch('{{ route('admin.editor.upload') }}', {
                        method: 'POST',
                        headers: {
                ...
                function insertPdfBlock(editor, pdfUrl, pdfName, sizeBytes) {
                    var safeName = escapeHtml(pdfName || 'document.pdf');
                    var sizeText = formatBytes(sizeBytes);
                    var subline = 'Click open to view in new tab' + (sizeText ? ' • ' + escapeHtml(sizeText) : '');
                    ...
                    var html =
                        '<div class="editor-pdf-block" style="margin:14px 0;border:1px solid #e5e7eb;border-radius:12px;overflow:hidden;background:#fff;">' +
                                ...
                                '<div style="display:flex;align-items:center;gap:10px;min-width:0;">' +
                                    ...
                                    '<div style="min-width:0;">' +
                                        '<div style="font-weight:700;color:#0f172a;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:520px;">' +
                                            safeName +
                                        '</div>' +
                                        '<div style="font-size:12px;color:#64748b;">' + subline + '</div>' +
                                    '</div>' +
                                '</div>' +
                            ...
                            '<div style="padding:0;margin:0;">' +
                                '<iframe src="' + pdfUrl + '" style="width:100%;height:800px;border:0;display:block;" loading="lazy"></iframe>' +
                            '</div>' +
                ...
                function initTinyMceFor(selectorRoot) {
                    ...
                    textareas.forEach(function(el) {
                        ...
                        tinymce.init({
                            ...
                            /* ✅ Added pdfUpload button to toolbar */
                            toolbar: 'undo redo | styles | bold italic underline | forecolor backcolor | alignleft aligncenter alignright | bullist numlist | table | link image media | pdfUpload | code fullscreen',
                                ...
                                editor.ui.registry.addButton('pdfUpload', {
                                    ...
                                    onAction: function() {
                                        ...
                                        input.onchange = function() {
                                            ...
                                            uploadFileToEditorEndpoint(file)
                                                .then(function(json) {
                                                    if (!json || !json.location) return;

                                                    var pdfUrl = json.location;
                                                    var pdfName = json.name ? json.name : (file.name ? file.name : 'document.pdf');
                                                    var sizeBytes = (typeof json.size_bytes !== 'undefined' && json.size_bytes !== null)
                                                        ? Number(json.size_bytes)
                                                        : Number(file.size);

                                                    insertPdfBlock(editor, pdfUrl, pdfName, sizeBytes);
                                                })
                                                .catch(function() {});
                                        };
